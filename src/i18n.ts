...
export type Locale = keyof I18n;

const defaultLocale: Locale = "en" as const;
export const locale = atom(defaultLocale as Locale);
 ...
 */
export const t = <P extends Path<I18n[typeof defaultLocale]>>(path: P, react = true) => {
	if (react) {
		return get(useStore(i18n)[useStore(locale)], path);
	}
	return get(i18n.get()[locale.get()], path);
};
